...
from .encode import SMTEncoder, MXEncoder, ILPEncoder
from .explain import SMTExplainer, MXExplainer
from .explain2 import ILPExplainer
import numpy as np
import os
import resource
...
class XGBooster(object):
        ...
        if from_data:
            ...
            if (self.use_categorical):
                ...
                for i in self.categorical_features:
                    self.encoder.update({i: OneHotEncoder(categories='auto', sparse_output=False)})#,
    ...
    def encode(self, test_on=None):
        ...
        if self.options.encode in ('mx', 'mxe', 'maxsat', 'mxint', 'mxa'):
            ...
            self.mxe = encoder
        elif self.options.encode in ('milp'):
            encoder = ILPEncoder(self.model, self.feature_names, self.num_class, self)
        else:  # smt or smtbool
        ...
        if use_lime:
            ...
            if 'x' not in dir(self):
                if self.options.encode in ('mx', 'mxe', 'maxsat', 'mxint', 'mxa'):
                    ...
                    self.x = ILPExplainer(self.enc, self.intvs, self.imaps,
                            self.ivars, self.feature_names, self.num_class,
                            self.options, self)
                else:
    ...
    def validate(self, sample, expl, exp_type='con'):
        """
        ...
        if exp_type == 'abd':
            ...
            # XH: validate mcs
            return self.v.validate_mcs(np.array(sample), expl)
    ...
    def save_model_for_compatibility(self, pickle_model_path: str, out_base: str):
        """
        Load a previously pickled XGBoost model, extract its Booster,
        and save out the native .model, per-tree JSON dump,
        and the training parameters in JSON form for HIS project compatibility.
        """
        # 1) Load the pickled model
        model = self.pickle_load_file(pickle_model_path)
        booster = model if isinstance(model, xgb.Booster) else model.get_booster()

        # 2) Build output base name if out_base is a directory
        if os.path.isdir(out_base):
            stem = os.path.splitext(os.path.basename(pickle_model_path))[0]
            stem = stem.replace('.mod', '')
            out_base = os.path.join(out_base, stem)

        os.makedirs(os.path.dirname(out_base), exist_ok=True)

        # 3) Save native XGBoost model
        model_path = out_base + ".model"
        booster.save_model(model_path)
        print(f"[✔] Exported booster to {model_path}")

        # 4) Optional: Dump each tree as JSON lines
        json_path = out_base + ".json"
        with open(json_path, "w") as fp:
            for tree_json in booster.get_dump(with_stats=True, dump_format="json"):
                fp.write(tree_json + "\n")
        print(f"[✔] Wrote per-tree JSON dump to {json_path}")

        # 5) Save the training parameters
        #    Use model.get_xgb_params() to capture all hyperparameters
        if hasattr(model, 'get_xgb_params'):
            params = model.get_xgb_params()
        else:
            params = {}
        params_path = out_base + ".params.json"
        with open(params_path, "w") as fp:
            json.dump(params, fp, indent=2)
        print(f"[✔] Wrote training parameters to {params_path}")
