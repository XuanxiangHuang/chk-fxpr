...
class MXEncoder(SMTEncoder):
    ...
    def encode(self):
        ...
        # creating initially empty class encodings
        self.enc = {}
        for j in range(self.nofcl):
    ...
    def test_sample(self, sample):
        ...
        #         'wrong prediction: {0} vs {1}'.format(cscores, escores)
        assert all(map(lambda c, e: abs(Decimal(c) - e) <= Decimal(0.01), cscores, escores)), \
            'wrong prediction: {0} vs {1}'.format(cscores, escores)
...
#
#==============================================================================
class ILPEncoder(object):
    """
        Encoder of XGBoost tree ensembles into ILP.
    """
    ...
    def traverse(self, tree, tvar, prefix=[]):
        ...
        if tree.children:
            bvar = self.encode_node(tree)

            self.traverse(tree.children[0], tvar, prefix + [(bvar,  1)])
            self.traverse(tree.children[1], tvar, prefix + [(bvar, -1)])
        else:  # leaf node
            if prefix:
                path = '_'.join(['path'] + ['{0}{1}'.format('p' if x[1] > 0 \
                        else 'n', x[0]) for x in sorted(prefix)])

                if path not in self.idmgr.obj2id:
                    self.idmgr.obj2id[path] = -1
                    self.oracle.variables.add(names=[path], types='B')

                    v = [p[0] for p in prefix]
                    c = [1 if p[1] > 0 else -1 for p in prefix]
                    r = len([p for p in prefix if p[1] > 0])

                    self.oracle.indicator_constraints.add(indvar=path,
                            lin_expr=[v, c], sense='G', rhs=r,
                            indtype=self.oracle.indicator_constraints.type_.iff)

                self.oracle.indicator_constraints.add(indvar=path,
                        lin_expr=[[tvar], [1]], sense='E', rhs=tree.values)
            else:
                self.oracle.linear_constraints.add(lin_expr=[[[tvar], [1]]],
                        senses=['E'], rhs=[tree.values])
    ...
    def encode_node(self, node):
        ...
        # introducing a fresh indicator variable if necessary
        if tuple([node.name, node.threshold]) not in self.idmgr.obj2id:
            bvar = 'bvar{0}'.format(self.idmgr.id(tuple([node.name, node.threshold])))
            self.oracle.variables.add(names=[bvar], types='B')

            if self.intvs:
                d = self.imaps[f][v] + 1
                pos, neg = self.ivars[f][:d], self.ivars[f][d:]

                self.oracle.indicator_constraints.add(indvar=bvar,
                        lin_expr=[pos, [1] * len(pos)], sense='G',
                        rhs=1,
                        indtype=self.oracle.indicator_constraints.type_.iff,
                        complemented=0)

                self.oracle.indicator_constraints.add(indvar=bvar,
                        lin_expr=[neg, [1] * len(neg)], sense='G',
                        rhs=1,
                        indtype=self.oracle.indicator_constraints.type_.iff,
                        complemented=1)
            else:
    ...
    def compute_intervals(self):
        ...
        for feat, intvs in six.iteritems(self.intvs):
            ...
            for i, ub in enumerate(intvs):
                ...
                ivar = '{0}_intv{1}'.format(feat, i)
                self.oracle.variables.add(names=[ivar], types='B')
                self.ivars[feat].append(ivar)
    ...
    def encode(self):
        ...
        # traverse all trees and extract all possible intervals
        # for each feature
        if self.optns.encode == 'ilpbool':
            self.compute_intervals()
        else:
            # feature variables
            fvars, lbs, ubs, types = [], [], [], []
            for fvar in self.xgb.extended_feature_names_as_array_strings:
                fvars.append(fvar)
                if '_' not in fvar:
                    lbs.append(-cplex.infinity)
                    ...
                    ubs.append(1)
                    types.append('B')
            self.oracle.variables.add(names=fvars, lb=lbs, ub=ubs, types=types)

        # introducing class score variables
        csum = []
        for j in range(self.nofcl):
            n = ['class{0}_score'.format(j)]
            l, u = [-cplex.infinity], [cplex.infinity]

            self.oracle.variables.add(names=n, lb=l, ub=u, types='C')
            csum.append(tuple([n[0], []]))
        ...
        for i, tree in enumerate(self.ensemble.trees):
            ...
            # encoding the tree
            # first, new variable for the tree's score
            n = ['tr{0}_score'.format(i + 1)]
            l, u = [-cplex.infinity], [cplex.infinity]
            self.oracle.variables.add(names=n, lb=l, ub=u, types='C')

            # traverse the tree and encode the paths
            self.traverse(tree, n[0], prefix=[])

            # this tree contributes to class with clid
            csum[clid][1].append(n[0])

        # encoding the sums
        for i, pair in enumerate(csum):
            c, ts = pair

            # -cvar + sum_of_tvars = 0.0
            eql = [[c] + ts, [-1] + [1] * len(ts)]
            rhs = [0]

            cnames = ['class_eq{0}'.format(i)]
            senses = ['E']
            constr = [eql]

            self.oracle.linear_constraints.add(lin_expr=constr, senses=senses,
                    rhs=rhs, names=cnames)
        ...
        for f in self.xgb.extended_feature_names_as_array_strings:
            if '_' in f:
                categories[f.split('_')[0]].append(f)

        for c, feats in six.iteritems(categories):
            eql = [[feats], [1] * len(feats)]
            rhs = [1]
            cnames = ['{0}_oneval'.format(c)]
            senses = ['E']
            constr = [eql]
            self.oracle.linear_constraints.add(lin_expr=constr, senses=senses,
                    rhs=rhs, names=cnames)
        ...
        if self.optns.verb:
            print('encoding vars:', self.oracle.variables.get_num())
            print('encoding asserts:', self.oracle.linear_constraints.get_num() + self.oracle.indicator_constraints.get_num())

        return self.oracle, self.intvs, self.imaps, self.ivars
    ...
    def test_sample(self, sample):
        ...
        if not self.intvs:
            for feat, fval in zip(self.xgb.extended_feature_names_as_array_strings, sample_internal):
                eql = [[feat], [1.0]]
                rhs = [float(fval) if '_' not in feat else int(fval)]

                cnames = ['hypo_{0}'.format(feat)]
                senses = ['E']
                constr = [eql]
        ...
        if self.oracle.solution.is_primal_feasible():
            ...
            for c in range(self.nofcl):
                val = float(model.get_values('class{0}_score'.format(c)))
                escores.append(val)
        ...
        for h in hypos:
            self.oracle.linear_constraints.delete(h)
    ...
    def save_to(self, outfile):
        ...
        if outfile.endswith('.txt'):
            outfile = outfile[:-3] + 'lp'

        # saving the encoding
        self.oracle.write(outfile, 'lp')
    ...
    def access(self):
        ...
        return self.oracle, self.intvs, self.imaps, self.ivars, self.feats, self.nofcl
