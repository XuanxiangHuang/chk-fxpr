...
from ortools.linear_solver.pywraplp import LinearExpr
...
class MXExplainer(object):
    """
        An MaxSAT-inspired minimal explanation extractor for XGBoost models.
    """
    ...
    def predict(self, sample):
        ...
        self.ord = sorted(scores, key=lambda i: scores[i], reverse=True)[0:]
    ...
    def explain(self, sample, smallest, expl_ext=None, prefer_ext=False):
        ...
        if self.verbose:
            ...
            if self.optns.xtype in ('contrastive', 'con') and smallest:
                print('  mmcs rtime: {0:.2f}'.format(self.mmcs_runtime))
                print('  mmcs round: {0}'.format(self.mmcs_round))
                print('  solver: {0}'.format(self.optns.solver))
    ...
    def _mmcs(self, heuristic=True, l0=False, l1=True):
        """
            XH: Compute minimum/smallest CXp
        """
        # compute unit-size CXp, if they exist, then simply return
        # otherwise, compute the smallest CXp

        exps = []
        rtimes = []
        min_exp = None
        self.mmcs_runtime = None            # runtime of the smallest CXp
        self.mmcs_round = 0                 # round of the smallest CXp
        lb = len(self.hypos)
        enc = self.xgb.mxe.enc
        labels = self.ord if heuristic else enc.keys()
        ...
        for cid in labels:
            ...
            wcnf = WCNF()
            self.mmcs_round += 1
            for cl in enc[cid].formula:
                wcnf.append(cl)
            if len(enc) > 2:
                for cl in enc[self.out_id].formula:
                    wcnf.append(cl)

            ##################################################
            # adding soft clauses and recording all the leaf values
            # all leaves to be used in the formula, am1 constraints
            wghts, lhs = [], []
            ub = 0

            for label in (cid, self.out_id):
                if label != self.out_id:
                    coeff = 1
                    ...
                    if len(enc) > 2:
                        ...
                        continue
                k = 0
                for tree in enc[label].trees:
                    for i in range(tree[0], tree[1]):
                        lit, wght = enc[label].leaves[i]
                        wghts.append(wght * coeff * (10 ** self.optns.relax))
                        lhs.append(lit)
                    ub += max(w*(int(l/abs(l))) for w,l in zip(wghts[k:], lhs[k:]))
                    k += tree[1] - tree[0]

            # create a PB constraint
            pb_lin = Bunch(lhs=lhs, coeffs=wghts, comp='>=', rhs=0, lb=0, ub=ub)
            pb_cnf = PBEnc.atleast(lits=lhs, weights=wghts, bound=0, top_id=enc[cid].formula.nv + 1)

            if l0:
                # soft clauses for L_0-distance CXp
                for h in self.hypos:
                    wcnf.append([h], weight=1)
            elif l1:
                x_min, x_max = np.min(self.xgb.X, axis=0), np.max(self.xgb.X, axis=0)
                for h in self.hypos:
                    f, i = self.xgb.mxe.vid2fid[h][0], self.v2feat[h]
                    splits = np.array([x_min[i]] + self.xgb.mxe.intvs[f][:-1] + [x_max[i]])
                    splits = (splits - x_min[i]) / (x_max[i] - x_min[i])
                    medians = [(splits[j + 1] + x) / 2.0 for j, x in enumerate(splits[:-1])]
                    mu = medians[self.in2Iid[i]]
                    wghts = 1.0 - abs(mu - medians)
                    wghts = np.round(wghts, self.optns.relax)
                    # print("weights: ", wghts)
                    # if len(wghts) == 2:
                    #     print("medians", medians)
                    #     print("splits", splits)
                    #     print(self.in2Iid[i], mu)

                    for l, w in zip(self.ivars[f], wghts):
                        wcnf.append([l], weight=w)
            else:
                raise NotImplementedError

            # wcnf.to_file('tmp.wcnf')
            # exit(0)

            # self.optns.solver = 'ortools'

            print("solver, heuristic, l0, l1: ", self.optns.solver, heuristic, l0, l1)

            if self.optns.solver == 'ortools':
                exp, rtime = self.ortools_oracle(wcnf, pb_lin, lb, l0=l0, l1=l1)
            else:
                # self.optns.solver == 'g3':
                exp, rtime = self.pysat_oracle(wcnf, pb_cnf, lb, l0=l0, l1=l1)
            ...
            if exp and (min_exp is None or len(exp) < len(min_exp)):
                min_exp, self.mmcs_runtime = exp, rtime
                lb = len(exp) - 1
    ...
    def ortools_oracle(self, wcnf, pb, lb, l0=True, l1=False):

        time_start = resource.getrusage(resource.RUSAGE_CHILDREN).ru_utime + \
                     resource.getrusage(resource.RUSAGE_SELF).ru_utime

        # toPBO(open('test.pbo', 'w'), wcnf, [pb], comment=None)
        # exit(0)
        def l2bv(l):
            if l > 0:
                bv = bvars[abs(l) - 1]
        ...
        bvars = [model.new_bool_var(name=f'x_{i + 1}') for i in range(wcnf.nv)]

        for cl in wcnf.hard:
            cl = [l2bv(l) for l in cl]
            model.add_bool_or(cl)

        model.add_linear_constraint(sum([w*l2bv(l) for l,w in zip(pb.lhs, pb.coeffs)]), pb.lb, pb.ub)
        model.add(sum([~l2bv(l) for l in self.hypos]) <= lb)

        if l0:
            model.minimize(sum([~l2bv(l) for l in self.hypos]))
        elif l1:
            model.maximize(sum([w * (10 ** self.optns.relax) * l2bv(cl[0]) for cl, w in zip(wcnf.soft, wcnf.wght)]))

        status = slv.solve(model)
        if status == cp_model.OPTIMAL or status == cp_model.FEASIBLE:
            exp = [self.v2cat[h] for h in self.hypos if slv.value(~l2bv(h))]
            print("OR-Tools obj val: ", slv.objective_value)
            print([cl[0] for cl in wcnf.soft if slv.value(l2bv(cl[0]))])
        else:
            exp = None

        time_end = resource.getrusage(resource.RUSAGE_CHILDREN).ru_utime + \
                   resource.getrusage(resource.RUSAGE_SELF).ru_utime

        return exp, time_end - time_start

    def pysat_oracle(self, wcnf, pb_cnf, lb, l0=True, l1=False):
        ...
        amk = CardEnc.atmost(lits=[-l for l in self.hypos], top_id=wcnf.nv + 1, bound=lb)
        for cl in amk.clauses:
            wcnf.append(cl)

        if l0:
            slv = RC2(wcnf, solver=self.optns.solver, adapt=False, exhaust=True, minz=False, trim=5)
        elif l1:
            slv = RC2Stratified(wcnf, solver=self.optns.solver, adapt=True, blo='full', exhaust=True, minz=True, trim=0)

        model = slv.compute()
        if model:
            exp = [self.v2cat[h] for h in self.hypos if model[abs(h) - 1] == -h]
            print("RC2.cost: ", slv.cost)
            print([cl[0] for cl in wcnf.soft if model[abs(cl[0])-1] == cl[0]])
        else:
            exp = None
...
def toPBO(fpointer, wcnf, pb_constrs, comment=None):
    """
        save encoding into PBO format
    """
    nc = len(wcnf.hard) + len(pb_constrs)
    if comment is not None:
        print('\n'.join(comment), file=fpointer)
        print('*', file=fpointer)
        nc += len(comment)
    else:
        print(f'* #variable= {wcnf.nv} #constraint= {nc}', file=fpointer)
        nc += 1

    def __str__(c):
        return (' '.join(map(fmt_lit, zip(c.lhs, c.coeffs)))) + f' {c.comp} {c.rhs} ;'
    ...
    def fmt_lit(wl):
        ...
        return f'{w} ~x{-x}'
    # objective function
    print(('min: '+' '.join(map(fmt_lit, zip([-cl[0] for cl in wcnf.soft], [1] * len(wcnf.soft)))) + ' ;'), file=fpointer)
    for cl in wcnf.hard:
