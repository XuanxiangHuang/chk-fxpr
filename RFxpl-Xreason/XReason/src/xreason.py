...
if __name__ == '__main__':
    ...
    if options.files:
        ...
        if options.train:
            ...
            # `model` is your XGBClassifier instance
            n_trees = model.n_estimators

            for i in range(n_trees):
                plt.figure(figsize=(20, 10))
                # plot_tree will internally grab the Booster for you
                plot_tree(model, num_trees=i, rankdir='TB')
                plt.savefig(f"xgb_tree_{i + 1}.png", dpi=300, bbox_inches='tight')
                plt.close()
            #
