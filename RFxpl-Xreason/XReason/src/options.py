...
class Options(object):
    ...
    def parse(self, command):
        ...
        if self.encode == 'none':
            self.encode = None
        elif self.encode in ('mx', 'mxe', 'maxsat', 'mxint', 'mxa') and self.solver in ('cvc5', 'mathsat', 'yices', 'z3', 'g3', 'ortools'):
            # setting the default solver for the mxreasoning-based oracle
            self.solver = 'g3'
        elif self.encode in ('milp') and self.solver in ('cplex'):
            self.solver = 'cplex'
    ...
    def usage(self):
        ...
        print('                                   Available values (smt): cvc5, mathsat, yices, z3 (default = cvc5)')
