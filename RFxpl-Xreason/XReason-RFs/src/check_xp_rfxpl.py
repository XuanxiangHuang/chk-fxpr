...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Check explanations from RFxpl')
    ...
    for data, depth in zip(datasets, depths):
        ...
        log_path = f"../../RFxpl/results/pyxai_vs_RFxpl/{xtype}/{base}.{num}.log"
        ...
        try:
            with open(log_path, "r") as f:
                for line in f:
                    ...
                    elif line.startswith(f"{xp_label}:"):
                        # strip off label and surrounding brackets
                        raw = line[len(xp_label) + 1:].strip().strip('[]')
        ...
        open(f"results/rfxpl_vs_xreason/{xtype}/{base}.{num}.log", 'w').write(output_str)
