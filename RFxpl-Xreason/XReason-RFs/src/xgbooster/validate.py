...
class SMTValidator(object):
    ...
    def __init__(self, formula, feats, nof_classes, xgb):
        ...
        # updated 2025-10-08: this is missing if we want to verify the encoding other than the model?
        self.intvs = self.xgb.intvs
        self.ivars = self.xgb.ivars
    ...
    def prepare(self, sample, expl):
        ...
        if self.oracle.solve([self.selv] + self.rhypos):
            ...
            # print("model: ", [(var, val) for var,val in model if 'class' in var.symbol_name()])
        else:
        ...
        # updated 2025-10-08
        outvals = np.array([float(model.get_py_value(o)) for o in self.outs])

        # argmax (ties → first max).
        true_output = int(np.argmax(outvals))

        # print("outvals: ", outvals)
        # print("prediction of enc: ", true_output)
        ...
        for i in range(len(self.outs)):
            ...
            if i < true_output:
                disj.append(GE(self.outs[i], self.outs[true_output]))
            elif i > true_output:
                disj.append(GT(self.outs[i], self.outs[true_output]))
                # print(disj)
        ...
        self.rhypos = hypos
        # print(self.rhypos, complement_hypos)
        # update:
        self.complement_hypos = complement_hypos
    ...
    def validate(self, sample, expl):
        ...
        if self.oracle.solve([self.selv] + self.rhypos):
            ...
            # print([(v,self.xgb.intvs[i][j]) for i in self.xgb.ivars for j,v in enumerate(self.xgb.ivars[i]) if model.get_py_value(v)])
            # outvals = [float(model.get_py_value(o)) for o in self.outs]
            # maxoval = max(zip(outvals, range(len(outvals))))

            outvals = np.array([float(model.get_py_value(o)) for o in self.outs])
            # argmax → index of the maximum (ties → first max)
            true_output = int(np.argmax(outvals))
            # keep a (score, index) tuple for the existing debug print
            maxoval = (float(outvals[true_output]), true_output)

            # print("########## validate inpvals ", inpvals)
            # print("########## validate outvals ", outvals)
            # print("########## validate maxoval ", maxoval)

            inpvals = self.xgb.transform_inverse(np.array(inpvals))[0]
            # self.coex = tuple([inpvals, maxoval[1]])
            self.coex = (inpvals, true_output)
            ...
            real_expl = sorted([int(s.symbol_name().split('_f')[1]) for s in self.rhypos])
        ...
        # return self.coex
        return valid, real_expl

    def validate_mcs(self, sample, expl):
        """
            validating the minimal correction set
        """

        self.time = resource.getrusage(resource.RUSAGE_CHILDREN).ru_utime + \
                resource.getrusage(resource.RUSAGE_SELF).ru_utime

        # get complement of mcs
        all_indices = set(self.ftids.values())
        complement = sorted(all_indices - set(expl))

        # adapt the solver to deal with the current sample
        self.prepare(sample, complement)

        # if UNsatisfiable, then there is a core
        if not self.oracle.solve([self.selv] + self.rhypos):
            print('CXp is incorrect')
            return False, []
        # otherwise, check if it is subset-minimal
        valid = True
        complement_hypos = self.complement_hypos[:]
        for h in self.complement_hypos:
            to_test = self.rhypos + [h]
            if self.oracle.solve([self.selv] + to_test):
                self.rhypos.append(h)
                complement_hypos.remove(h)
                valid = False
        real_expl = sorted([int(s.symbol_name().split('_f')[1]) for s in complement_hypos])
