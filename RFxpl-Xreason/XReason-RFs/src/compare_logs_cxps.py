#!/usr/bin/env python3
"""
Compare RFxpl and Xreason log files to check agreement on PyXAI CXp explanations.
"""
...
XREASON_BASE = "results/pyxai_vs_xreason/con"
RFXPL_BASE = "../../RFxpl/results/pyxai_vs_RFxpl/con"
...
def parse_log_file(filepath):
    ...
    Returns a dict mapping instance string to its CXp value.
    """
    instances = {}
    ...
    while i < len(lines):
        ...
        if line.startswith('i:'):
            ...
            # Read the next few lines to find PyXAI and CXp
            j = i + 1
            while j < len(lines) and j < i + 20:  # Look ahead up to 20 lines
                ...
                elif next_line.startswith('CXp:'):
                    cxp = next_line[4:].strip()

                j += 1

            # Store the instance with its CXp
            if instance and cxp is not None:
                instances[instance] = {
                    'pyxai': pyxai,
                    'cxp': cxp
                }
...
def compare_logs(xreason_file, rfxpl_file):
    ...
    for instance in sorted(common_instances):
        xr_cxp = xreason_data[instance]['cxp']
        rf_cxp = rfxpl_data[instance]['cxp']
...
def main():
    ...
    print("=" * 80)
    print("RFxpl vs Xreason CXp Comparison Report")
    print("=" * 80)
    print()
    ...
    for dataset in datasets:
        ...
        if result['disagreements']:
            ...
            for d in result['disagreements']:
                ...
                print(f"    Xreason:  {d['xreason_cxp']}")
                print(f"    RFxpl:    {d['rfxpl_cxp']}")
                print()
