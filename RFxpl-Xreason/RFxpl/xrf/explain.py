...
class SATExplainer(object):
    ...
    def _mus(self):
        ...
        # Update:
        # print("_mus:", self.assums)
        # assert (not self.slv.solve(self.assums))
        # for i,h in enumerate(self.assums):
        #     if h > 0:
        #         to_test = self.assums[:i] + self.assums[(i + 1):]
        #         assert self.slv.solve(assumptions=to_test)
    ...
    def _mcs(self):
        ...
        # Update:
        # print("_mcs:", self.assums, mcs, [self.assums[i-1] for i in mcs])
...
#                 yield advx
    ...
    def validate(self, sample, expl, xtype='abd', min_max_dict=None, rfmd=None):
        """
            Hypotheses minimization.
        """

        self.prepare_selectors(sample)
        self.slv = Solver(name="glucose3", with_proof=True)

        if xtype == 'abd':
            xp_chk, cert, expl, wit, num_wits, num_proofs = self.validate_axp(sample, expl, min_max_dict, rfmd)
        elif xtype == 'con':
            xp_chk, cert, expl, wit, num_wits, num_proofs = self.validate_cxp(sample, expl, min_max_dict, rfmd)
        else:
            xp_chk, cert, expl, wit, num_wits, num_proofs = self.validate_maj(sample, expl)
        ...
        return xp_chk, cert, expl, wit, num_wits, num_proofs

    def validate_maj(self, sample, expl):
        """
            Validating the majority reason.
        """
        ...
        if self.slv.solve(assumptions=assums):
            print("MAJ is incorrect")
            # delete sat solver
            return False, []
        return True, expl

    def generate_wit(self, sample, model, min_max_dict):
        """
            Generating a witness for the counter-example.
        """
        wit = sample.copy()
        hypos = [model[abs(h) - 1] for h in self.assums]
        fid2hypo = {self.sel2fid[abs(h)]: model[abs(h) - 1] for h in self.assums}
        # Handle continuous features with intervals
        for f in self.enc.ivars:
            fid = int(f[1:])
            if len(self.enc.ivars[f]) == 0:
                # feature inessential for the prediction (never appear in TE)
                continue
            h = fid2hypo[fid]
            if h > 0:
                # feature fixed
                assert( model[abs(self.sel2v[h]) - 1] )
                j = int(self.enc.nameVar(self.sel2v[h]).split('_intv')[1])
                if j == 0:
                    assert sample[fid] <= self.enc.intvs[f][j]
                elif j == len(self.enc.intvs[f]) - 1:
                    assert sample[fid] > self.enc.intvs[f][j - 1]
                else:
                    assert (self.enc.intvs[f][j - 1] < sample[fid] <= self.enc.intvs[f][j])
            else:
                # feature free
                for l in self.enc.ivars[f]:
                    if model[abs(l) - 1] > 0:
                        j = int(self.enc.nameVar(l).split('_intv')[1])
                        if not j:
                            v = self.enc.intvs[f][j] - 1.0
                        elif j == len(self.enc.intvs[f]) - 1:
                            v = self.enc.intvs[f][j - 1] + 1.0
                        else:
                            v = (self.enc.intvs[f][j - 1] + self.enc.intvs[f][j]) / 2.0
                        # Clip the value to be within the meaningful range defined by min_max_dict
                        feature_min = min_max_dict[fid]['min']
                        feature_max = min_max_dict[fid]['max']
                        v = max(feature_min, min(v, feature_max))
                        wit[fid] = v

        # Handle categorical features (with One-Hot Encoding)
        for f in self.enc.categories:
            for l in self.enc.categories[f]:
                if model[abs(l) - 1] > 0:
                    v = self.enc.nameVar(l).split('_')[1]  # must check OHE
                    wit[fid] = v
                    raise NotImplementedError('OHE features are not supported yet')
        return wit

    def run_proof_checker(self, model_cnf, assums, proof_trace):
        """
            Running DRAT proof checker to validate the explanation.
        """
        # proof checker
        cnf = model_cnf.copy()
        ...
        drat_file = "example.drat"

        result = subprocess.run(
            ["./drat-trim", cnf_file, drat_file, '-f'],
            capture_output=True,
            text=True
        )

        # Combine stdout and stderr (drat-trim sometimes prints to both)
        ...
        if "s VERIFIED" in output:
            ...
            # print("Proof is NOT VERIFIED.")
            proof_verified = False
        else:
            print("drat-trim returned unexpected output:")
            print(output)
            proof_verified = None
        return proof_verified

    def validate_axp(self, sample, expl, min_max_dict, rfmd):
        """
            Validating the abductive explanation.
        """
        num_wits, num_proofs = 0, 0
        assert len(self.assums)
        self.assums = sorted(set(self.assums))
        assums = self.assums[:]
        ########## preprocessing ##########
        ...
        if self.slv.solve(assumptions=assums):
            model = self.slv.get_model()
            wit = self.generate_wit(sample, model, min_max_dict)
            num_wits += 1
            if rfmd.predict(wit) != rfmd.predict(sample):
                print("AXp is incorrect")
                return False, True, [], wit, num_wits, num_proofs
            else:
                print("check WAXp error: witness has the same prediction as the original sample!")
                return False, False, [], wit, num_wits, num_proofs
        else:
            # validate the proof
            proof_trace = self.slv.get_proof()
            num_proofs += 1
            proof_ret = self.run_proof_checker(self.cnf, assums, proof_trace)
            if proof_ret is False:
                print("check WAXp error: AXp proof is incorrect")
                return False, False, [], None, num_wits, num_proofs
            elif proof_ret is None:
                print("check WAXp error: AXp proof could not be verified")
                return False, False, [], None, num_wits, num_proofs
            # else: proof is verified

        # 2) try to remove fixed features
        for i, slt in enumerate(assums):
            if slt > 0:
                to_test = assums[:i] + assums[(i + 1):] + [-slt]
                with Solver(name="glucose3", bootstrap_with=self.cnf, with_proof=True) as solver:
                    if not solver.solve(assumptions=to_test):
                        # validate the proof
                        proof_trace = solver.get_proof()
                        num_proofs += 1
                        proof_ret = self.run_proof_checker(self.cnf, to_test, proof_trace)
                        if proof_ret is False:
                            print("check sub-min error: AXp proof is incorrect")
                            return False, False, [], None, num_wits, num_proofs
                        elif proof_ret is None:
                            print("check sub-min error: AXp proof could not be verified")
                            return False, False, [], None, num_wits, num_proofs
                        # else: proof is verified
                        assums[i] = -slt
                    else:
                        ...
                        num_wits += 1
                        if rfmd.predict(wit) == rfmd.predict(sample):
                            print("check sub-min error: witness has the same prediction as the original sample!")
                            return False, False, [], wit, num_wits, num_proofs

        real_expl = sorted([self.sel2fid[abs(h)] for h in assums if h > 0])
        valid = (len(real_expl) == len(expl))

        return valid, True, real_expl, None, num_wits, num_proofs

    def validate_cxp(self, sample, expl, min_max_dict, rfmd):
        """
            Validating the contrastive explanation.
        """
        num_wits, num_proofs = 0, 0
        assert len(self.assums)
        self.assums = sorted(set(self.assums))
        ...
        # pass a CNF formula
        self.slv.append_formula(self.cnf)
        # if the CNF encoding is SAT then we got a weak CXp,
        # otherwise not a weak CXp
        if not self.slv.solve(assumptions=assums):
            # validate the proof
            proof_trace = self.slv.get_proof()
            num_proofs += 1
            proof_ret = self.run_proof_checker(self.cnf, assums, proof_trace)
            if proof_ret is False:
                print("check WCXp error: CXp proof is incorrect")
                return False, False, [], None, num_wits, num_proofs
            elif proof_ret is None:
                print("check WCXp error: CXp proof could not be verified")
                return False, False, [], None, num_wits, num_proofs
            # else: proof is verified
            print("CXp is incorrect")
            return False, True, [], None, num_wits, num_proofs
        else:
            # check witness
            model = self.slv.get_model()
            wit = self.generate_wit(sample, model, min_max_dict)
            num_wits += 1
            if rfmd.predict(wit) == rfmd.predict(sample):
                print("check WCXp error: witness has the same prediction as the original sample!")
                return False, False, [], wit, num_wits, num_proofs

        # 2) try to remove freed features
        for i, slt in enumerate(assums):
            if slt < 0:
                to_test = assums[:i] + assums[(i+1):] + [-slt]
                with Solver(name="glucose3", bootstrap_with=self.cnf, with_proof=True) as solver:
                    if solver.solve(assumptions=to_test):
                        # check witness
                        assums[i] = -slt
                        model = solver.get_model()
                        wit = self.generate_wit(sample, model, min_max_dict)
                        ...
                        proof_ret = self.run_proof_checker(self.cnf, to_test, proof_trace)
                        if proof_ret is False:
                            print("check sub-min error: CXp proof is incorrect")
                            return False, False, [], None, num_wits, num_proofs
                        elif proof_ret is None:
                            print("check sub-min error: CXp proof could not be verified")
                            return False, False, [], None, num_wits, num_proofs
                        # else: proof is verified
