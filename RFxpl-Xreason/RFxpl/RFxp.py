...
if __name__ == '__main__':
    ...
    if options.files:
        ...
        if options.train:
            ...
            # save model for Sci-Kit-learn compatibility
            try:
                os.stat(f'../exported_rfs/{bench_dir_name}')
            except:
                os.makedirs(f'../exported_rfs/{bench_dir_name}')
            basename_sk = (os.path.join(f'../exported_rfs/{bench_dir_name}', bench_name +
                                        "_nbestim_" + str(options.n_estimators) +
                                        "_maxdepth_" + str(options.maxdepth)))
            cls.save_model(f"{basename_sk}.model")
            cls.save_params(f"{basename_sk}.params.json")

            # cls._pickle_save_file(f"{basename}.model", cls.forest)
            # params = cls.forest.get_params()
            # with open(f"{basename}.params.json", 'w') as fp:
            #     json.dump(params, fp, indent=2)
            #
            # from sklearn.tree import plot_tree
            # import matplotlib.pyplot as plt
            #
            # for i in range(len(cls.forest.estimators_)):
            #     # Plot the first tree of the Random Forest
            #     plt.figure(figsize=(20, 10))
            #     plot_tree(cls.forest.estimators_[i], filled=True)
            #
            #     # Save the plot as a PNG file
            #     plt.savefig(f"rf_tree_{i+1}.png", format="png")

            # Or specify a path like "/path/to/save/decision_tree_plot.png"
        ...
        if options.explain:
            ...
            if options.xnum > 1:
                ...
                xrf.validate(options.explain, expl, xtype=options.xtype)
